"""Vercel serverless function – refer-a-colleague form to email via Resend API."""
...
def format_apply_other(data):
    ...
    urgency = data.get("person_urgency", "")
    urgency_label = data.get("person_urgency_label", urgency)
    urgency_color = URGENCY_COLORS.get(urgency, "#6b7280")

    subject = f"PAPS Yönlendirme: {person_name} – Aciliyet: {urgency_label}"

    referrer_rows = (
        _row("Pegasus Çalışanı mı?", data.get("is_employee_label", data.get("is_employee", "")))
        + _row("Rolü / Departman", data.get("referrer_role_label", data.get("referrer_role", "")))
        + _row("Sicil No", data.get("referrer_id"))
        + _row("İlişki", data.get("referrer_relation"))
        + _row("Yönlendiren Ad", data.get("referrer_name"))
        + _row("Yönlendiren Tel", data.get("referrer_phone"))
        + _row("Yönlendiren E-posta", data.get("referrer_email"))
    )

    person_rows = (
        _row("Kişi Adı", person_name)
        + _row("Kişi E-posta", data.get("person_email"))
        + _row("Unvan", data.get("person_title_label", data.get("person_title", "")))
        + _row("Lokasyon", data.get("person_location_label", data.get("person_location", "")))
        + _row("Aciliyet", f'{_badge(urgency_label, urgency_color)}')
        + _row("Konu", data.get("person_topic_label", data.get("person_topic", "")))
        + _row("Detaylar", (data.get("reason", "") or "").replace("\n", "<br>"))
    )

    rows = (
        f'<tr><td colspan="2" style="padding:12px 0 4px;font-weight:700;font-size:15px;color:#1a1a2e;">Yönlendiren Bilgileri</td></tr>'
        + referrer_rows
        + f'<tr><td colspan="2" style="padding:16px 0 4px;font-weight:700;font-size:15px;color:#1a1a2e;">Yönlendirilen Kişi</td></tr>'
        + person_rows
    )

    return subject, _wrap_html("Yeni Yönlendirme Başvurusu", "Yönlendirme", "#7c3aed", rows)
...
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        try:
            subject, html = format_apply_other(data)
