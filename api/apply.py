"""Vercel serverless function – peer support application form to email via Resend API."""
...
def format_apply(data):
    ...
    urgency_label = data.get("urgency_label", urgency)
    urgency_color = URGENCY_COLORS.get(urgency, "#6b7280")

    subject = f"PAPS Başvuru: {name} – Aciliyet: {urgency_label}"
    rows = (
        _row("Ad Soyad", name)
        + _row("Unvan", data.get("title_label", data.get("title", "")))
        + _row("Lokasyon", data.get("location_label", data.get("location", "")))
        + _row("E-posta", data.get("email"))
        + _row("Telefon", data.get("phone"))
        + _row("Aciliyet", f'{_badge(urgency_label, urgency_color)}')
        + _row("Konu", data.get("topic_label", data.get("topic", "")))
        + _row("Peer Tercihi", data.get("peer_label", data.get("peer", "")))
        + _row("Detaylar", (data.get("message", "") or "").replace("\n", "<br>"))
    )
    return subject, _wrap_html("Yeni Peer Support Başvurusu", "Başvuru", "#e21f26", rows)
...
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        try:
            subject, html = format_apply(data)
